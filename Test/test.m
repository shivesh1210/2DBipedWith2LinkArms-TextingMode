...
% Load an already optimised set of parameters for simulation
 load('Optimisation Parameters Results\Optim_Parameters11_V14.mat');

Walk_Speed = 1.4;    % Set this according to the file loaded
...
% xstart = 0;
% [xstep1 zstep1 aviobj] = animation(poly(:,1:impactindex), xstart, zstart, aviobj);
% title('Impact');
% % pause(5);
% title('Step 2');
% [xstep2 zstep2 aviobj] = animation(poly(:,impactindex+1:end), xstep1, zstart, aviobj);
% aviobj = close(aviobj);
