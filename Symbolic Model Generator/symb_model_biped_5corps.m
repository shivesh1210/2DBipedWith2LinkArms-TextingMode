%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Symbolic Model for a 5 Link biped in 2D
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

clear all
clc
% This is for the biped model, with right foot on the ground, walking left to right. The model is 
% for five degrees of freedom of the robot, with Foot1 touching the ground
%

syms q1 q2 q3 q4 q5 q6 q7 q8 q9 qp1 qp2 real
...
ddq=[ddq1 ddq2 ddq3 ddq4 ddq5 ddq6 ddq7 ddq8 ddq9 ddqp2].';
...
%% Hip Coordinates DGM
% NOTE: Comment this part when building it for reduced coordinates
xh = -L1*sin(q1)-L2*sin(q2);
yh = L1*cos(q1)+L2*cos(q2);
dxh=jacobian(xh,q)*dq;
dyh=jacobian(yh,q)*dq;
ddxh=jacobian(dxh,q)*dq+jacobian(xh,q)*ddq;
ddyh=jacobian(dyh,q)*dq+jacobian(yh,q)*ddq;

%% Centre of gravity of links
G1 = [xh + L2*sin(q2) + s1*sin(q1), yh - L2*cos(q2) - s1*cos(q1)];
G2 = [xh + s2*sin(q2), yh - s2*cos(q2)];
G3 = [xh - s3*sin(q3), yh + s3*cos(q3)]; % Trunk
G4 = [xh + s4*sin(q4), yh - s4*cos(q4)];
G5 = [xh + L4*sin(q4) + s5*sin(q5), yh - L4*cos(q4) - s5*cos(q5)];
% Arm 1 
G6=[xh-sb*sin(q3)+s6*sin(q6),yh+sb*cos(q3)-s6*cos(q6)];     % Upper arm
G7=[xh-sb*sin(q3)+L6*sin(q6)+s7*sin(q7),yh+sb*cos(q3)-L6*cos(q6)-s7*cos(q7)];   % Lower arm
% Arm 2
G8=[xh-sb*sin(q3)+s8*sin(q8),yh+sb*cos(q3)-s8*cos(q8)];     % Upper arm
G9=[xh-sb*sin(q3)+L8*sin(q8)+s9*sin(q9),yh+sb*cos(q3)-L8*cos(q8)-s9*cos(q9)];   % Lower arm


%% Centre of gravity of feet
...
%point just under the heel of the feet touching ground
...
foot1_heel=[xh + L2*sin(q2) + L1*sin(q1) + hp*sin(qp1)-lp*cos(qp1), yh - L2*cos(q2) - L1*cos(q1) - hp*cos(qp1)-lp*sin(qp1)];
foot1_toe=[xh + L2*sin(q2) + L1*sin(q1) + hp*sin(qp1)+ld*cos(qp1), yh - L2*cos(q2) - L1*cos(q1) - hp*cos(qp1)+ld*sin(qp1)];

foot2_heel=[xh  + L4*sin(q4)+ L5*sin(q5) + hp*sin(qp2)-lp*cos(qp2), yh - L4*cos(q4) - L5*cos(q5) - hp*cos(qp2)-lp*sin(qp2)];
foot2_toe=[xh  + L4*sin(q4)+ L5*sin(q5) + hp*sin(qp2)+ld*cos(qp2), yh - L4*cos(q4) - L5*cos(q5) - hp*cos(qp2)+ld*sin(qp2)];
...
%% Calculation of Linear velocity of links of the robot
...
vxh=jacobian(xh,q)*dq;
vyh=jacobian(yh,q)*dq;
vfoot1=jacobian(FG1,q)*dq;
vfoot2=jacobian(FG2,q)*dq;
...
KE  =simple(KE1+KE2+KE3+KE4+KE5+KE6+KE7+KE8+KE9+KE_foot2);


%% Calculation of Potential Energy of the robot U=m*g*h
PE = m1*g*G1(2) + m2*g*G2(2) + m3*g*G3(2) + m4*g*G4(2)+ m5*g*G5(2) + m6*g*G6(2) + m7*g*G7(2) + m8*g*G8(2)+ m9*g*G9(2)+ mp*g*FG2(2);
PE=simple(PE);
%pretty(PE)
...
for k=1:n
   for j=1:n
      C(k,j)=0*g;
      for i=1:n
         C(k,j)=C(k,j)+(1/2)*(diff(D(k,j),q(i))+diff(D(k,i),q(j))-diff(D(j,i),q(k)))*dq(i);
      end
   end
end
C=simple(C);
E=[q(1) q(2) q(3) q(4) q(5) q(6) q(7) q(8) q(9) q(10)];
B=jacobian(E,q);
B=transpose(B);
...
% %Calcul de la vitesse absolue du centre de gravit� du tronc
% vitcmgtronc = [dq(5)-Pied1p(1);dq(6)-Pied1p(2)];
vitcmgtronc = vc3;
%% Velocities of extremities of feet
vfoot1_heel=jacobian(foot1_heel,q)*dq;
vfoot1_toe=jacobian(foot1_toe,q)*dq;

vfoot2_heel=jacobian(foot2_heel,q)*dq;
vfoot2_toe=jacobian(foot2_toe,q)*dq;
...
%% Calculation of Jacobian matrix of foot_2 Ac2
...
% 
% save work_symb_model_biped_abs_red
% fcn_name = 'dyn_mod_biped_abs_red';
